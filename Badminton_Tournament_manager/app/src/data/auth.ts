...
export type UserRole = 'admin' | 'btc' | 'referee' | 'athlete' | null
...
export function authApprove(userId: string, assignedRole: string): void {
  saveUsers(getUsers().map(u => u.id === userId ? { ...u, role: assignedRole as UserRole, status: 'approved' as const } : u))
}

export function authReject(userId: string): void {
  saveUsers(getUsers().map(u => u.id === userId ? { ...u, status: 'rejected' as const } : u))
}

export function authChangeRole(userId: string, newRole: string): void {
  saveUsers(getUsers().map(u => u.id === userId ? { ...u, role: newRole as UserRole } : u))
}

export function useAuth() {
  const [session, setSession] = useState<Session | null>(getSession)

  const login = (credential: string, password: string): { ok: boolean; error?: string } => {
    ...
    const s: Session = { userId: user.id, role: user.role, name: user.name, email: user.email }
    persistSession(s)
    setSession(s)
    return { ok: true }
  }
  ...
  const register = ({ name, email, phone, password, requestedRole }: {
    name: string; email: string; phone?: string; password: string; requestedRole: string
